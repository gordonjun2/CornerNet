...
RUN pip install --no-cache-dir python-dateutil==2.7.2 && \
    ...
    pip install --no-cache-dir Cython==0.27.3 && \
    pip install --no-cache-dir kiwisolver==1.0.1 && \
    pip install --no-cache-dir matplotlib==2.2.2 && \
    pip install --no-cache-dir ninja==1.8.2 && \
    pip install --no-cache-dir olefile==0.45.1 && \
    pip install --no-cache-dir Pillow==5.1.0 && \
    pip install --no-cache-dir protobuf==3.5.2 && \
    pip install --no-cache-dir pycparser==2.18 && \
    pip install --no-cache-dir pyparsing==2.2.0 && \
    pip install --no-cache-dir pyqt5==5.9.2 && \
    ...
    pip install --no-cache-dir sip==4.19.8 && \
...
# Compiling NMS for CornerNet
WORKDIR /home/root/CornerNet/external
RUN make

# Installing MS COCO APIs for CornerNet
